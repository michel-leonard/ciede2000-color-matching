...
For cross-language verification purposes, the generic implementation can also perform ΔE<sub>00</sub> calculations with arbitrary precision.

**Example** : `ciede_2000(87,-29,-110,69,-83,-110.18)` equals `18.870419634043393742716037160437508801597912526020124112418`.

A source code containing decimal literals like `0.32` can propagate small but significant binary rounding errors in generic-precision arithmetic. For example `BigFloat(0.32) != BigFloat(8//25)`, so the generic function ΔE<sub>00</sub> in Julia includes constants written as fractions like `8//25`.
...
Where the main files involved are [ciede-2000-driver.jl](ciede-2000-driver.jl#L92) for calculations and [test-jl.yml](../../.github/workflows/test-jl.yml) for automation.
</details>
...
The arbitrary-precision Julia implementation of `ciede_2000` inherits the validity of the 64-bit Julia version through rigorous validation.

First, the 64-bit Julia implementation was validated against the 64-bit C99 version, ensuring compliance with standards.

Next, the Julia implementation, using the `AbstractFloat` type to support arbitrary precision, was tested against the `bc` (Basic Calculator) implementation. This validation, detailled [here](../bc#δe2000--accurate-fast-bc-powered), showed a maximum deviation between between Julia and `bc` not exceeding `3.9e-42`.

This agreement confirms that arbitrary-precision is correctly implemented in Julia, fast (25× faster than `bc`) and precise.

> In summary, this Julia version combines rigorous validation against the C and `bc` implementations across all supported precisions.
</details>
...
[Calculator](https://michel-leonard.github.io/ciede2000-color-matching/lab-color-calculator.html?L1=53&a1=-96&b1=-45&L2=12.5&a2=-106&b2=-26.1) — [Workflow Details](../../.github/workflows#workflow-details) — 🌐 [Suitable in 30+ Languages](../../#implementations)
