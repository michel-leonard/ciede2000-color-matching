...
procedure main is
	...
	begin
		...
		if a_1 = 0.0 and b_1 = 0.0 then
			h_1 := 0.0;
		else
			h_1 := Math.Arctan(b_1, a_1 * n);
			if h_1 < 0.0 then h_1 := h_1 + 2.0 * Ada.Numerics.Pi; end if;
		end if;
		if a_2 = 0.0 and b_2 = 0.0 then
			h_2 := 0.0;
		else
			h_2 := Math.Arctan(b_2, a_2 * n);
			if h_2 < 0.0 then h_2 := h_2 + 2.0 * Ada.Numerics.Pi; end if;
		end if;
		if h_1 < h_2 then n := h_2 - h_1; else n := h_1 - h_2; end if;
		-- Cross-implementation consistent rounding.
		if Ada.Numerics.Pi - 1.0E-14 < n and n < Ada.Numerics.Pi + 1.0E-14 then
			n := Ada.Numerics.Pi;
		end if;
		...
		h_m := (h_1 + h_2) * 0.5;
		h_d := (h_2 - h_1) * 0.5;
		if Ada.Numerics.Pi < n then
			if 0.0 < h_d then
				h_d := h_d - Ada.Numerics.Pi;
			else
				h_d := h_d + Ada.Numerics.Pi;
			end if;
			h_m := h_m + Ada.Numerics.Pi;
		end if;
		p := 36.0 * h_m - 55.0 * Ada.Numerics.Pi;
		n := (c_1 + c_2) * 0.5;
		n := n * n * n * n * n * n * n;
		-- The hue rotation correction term is designed to account for the
		-- non-linear behavior of hue differences in the blue region.
		r_t := -2.0 * 	Math.Sqrt(n / (n + 6103515625.0)) *
				Math.Sin(Ada.Numerics.Pi / 3.0 *
				Math.Exp(p * p / (-25.0 * Ada.Numerics.Pi * Ada.Numerics.Pi)));
		n := (l_1 + l_2) * 0.5;
		n := (n - 50.0) * (n - 50.0);
		-- Lightness.
		...
		t := 1.0 +	0.24 * Math.Sin(2.0 * h_m + Ada.Numerics.Pi / 2.0) +
				0.32 * Math.Sin(3.0 * h_m + 8.0 * Ada.Numerics.Pi / 15.0) -
				0.17 * Math.Sin(h_m + Ada.Numerics.Pi / 3.0) -
				0.20 * Math.Sin(4.0 * h_m + 3.0 * Ada.Numerics.Pi / 20.0);
		n := c_1 + c_2;
		-- Hue.
