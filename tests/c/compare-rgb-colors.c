...
// L1 = 40.9   a1 = 55.9   b1 = 2.4
// L2 = 40.8   a2 = 50.0   b2 = -2.1
// CIE ΔE00 = 2.8019805257 (Bruce Lindbloom, Netflix’s VMAF, ...)
// CIE ΔE00 = 2.8019940237 (Gaurav Sharma, OpenJDK, ...)
...
static void rgb_to_xyz(double r, double g, double b, double *x, double *y, double *z) {
	...
	r = r > 0.040448236276933 ? pow((r + 0.055) / 1.055, 2.4) : r / 12.92;
	g = g > 0.040448236276933 ? pow((g + 0.055) / 1.055, 2.4) : g / 12.92;
	b = b > 0.040448236276933 ? pow((b + 0.055) / 1.055, 2.4) : b / 12.92;
...
static void xyz_to_rgb(double x, double y, double z, double *r, double *g, double *b) {
	...
	*r = *r > 0.0031306684424956 ? 1.055 * pow(*r, 1.0 / 2.4) - 0.055 : 12.92 * *r;
	*g = *g > 0.0031306684424956 ? 1.055 * pow(*g, 1.0 / 2.4) - 0.055 : 12.92 * *g;
	*b = *b > 0.0031306684424956 ? 1.055 * pow(*b, 1.0 / 2.4) - 0.055 : 12.92 * *b;
}
...
// The goal of this demo in C is to use the CIEDE2000 function to compare two RGB colors.

#include <stdio.h>

int main(void) {
	int r_1 = 0, g_1 = 0, b_1 = 139; // darkblue
	int r_2 = 0, g_2 = 0, b_2 = 128; // navy
	...
	printf("RGB(%-3d, %-3d, %d) -> LAB(L: %-6.4g, a: %-6.4g, b: %.4g)\n", r_1, g_1, b_1, l_1, a_1, b_1_lab);
	printf("RGB(%-3d, %-3d, %d) -> LAB(L: %-6.4g, a: %-6.4g, b: %.4g)\n", r_2, g_2, b_2, l_2, a_2, b_2_lab);

	// 2. ΔE2000
	const double delta_e = ciede_2000(l_1, a_1, b_1_lab, l_2, a_2, b_2_lab);
	printf("Delta E 2000 between the two colors: %.4g", delta_e);

	// This shows a ΔE2000 of 15.56
}

// This file is named "compare-rgb-colors.c"

// The compilation is done using GCC or CLang :
// - gcc -std=c99 -Wall -Wextra -pedantic -Ofast -o ciede-2000-demo compare-rgb-colors.c -lm
// - clang -std=c99 -Wall -Wextra -pedantic -Ofast -o ciede-2000-demo compare-rgb-colors.c -lm
