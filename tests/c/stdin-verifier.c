...
// The verification is performed here in C99. It reads the CSV data from STDIN and prints a completion message.

// CSV files can also be generated and validated using the Large-Scale Validator,
// available at: https://michel-leonard.github.io/ciede2000-color-matching
...
int main(void) {
	...
	while (fgets(buf, 255, stdin)) {
		...
		if (c == EOF)
			...
			ungetc(c, stdin);
		const char *t_1 = strtok(buf, ",");
		const char *t_2 = strtok(0, ",");
		const char *t_3 = strtok(0, ",");
		const char *t_4 = strtok(0, ",");
		const char *t_5 = strtok(0, ",");
		const char *t_6 = strtok(0, ",");
		const char *t_7 = strtok(0, "\n");
		if (t_1 && t_2 && t_3 && t_4 && t_5 && t_6 && t_7) {
			const double l_1 = strtod(t_1, 0);
			const double a_1 = strtod(t_2, 0);
			const double b_1 = strtod(t_3, 0);
			const double l_2 = strtod(t_4, 0);
			const double a_2 = strtod(t_5, 0);
			const double b_2 = strtod(t_6, 0);
