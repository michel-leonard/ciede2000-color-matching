...
This implementation of the ΔE2000 function in Mathematica was tested on my local machine, and does not benefit, like other programming languages, from an automated testing workflow on GitHub Actions (because it's a proprietary language). The [ciede-2000-driver.wl](ciede-2000-driver.wl#L79) file is provided in this directory so that you can perform similar tests yourself by processing a CSV file. Here's a step-by-step guide to my tests :

- Installation of **GCC** : `command -v gcc > /dev/null || { sudo apt-get update && sudo apt-get install gcc ; }`
- Driver compiled in **C99** : `gcc -std=c99 -Wall -pedantic -Ofast -ffast-math -g tests/c/ciede-2000-driver.c -o ciede-2000-driver -lm`
...
🌐 [Suitable in 30+ Languages](../../#implementations)
