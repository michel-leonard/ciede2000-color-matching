...
// L1 = 77.17          a1 = 83.7994        b1 = -36.178
// L2 = 77.17          a2 = 83.7994        b2 = -36.23
// CIE ΔE2000 = ΔE00 = 0.01696799612

// L1 = 60.0           a1 = 113.0          b1 = -90.25
// L2 = 60.0           a2 = 113.0          b2 = -89.9783
// CIE ΔE2000 = ΔE00 = 0.06966502299

// L1 = 72.9204        a1 = 36.8           b1 = -7.5288
// L2 = 72.9204        a2 = 43.408         b2 = -7.5288
// CIE ΔE2000 = ΔE00 = 2.38659021819

// L1 = 55.117         a1 = -100.0         b1 = 92.0
// L2 = 58.0           a2 = -100.0         b2 = 92.0
// CIE ΔE2000 = ΔE00 = 2.66628516352

// L1 = 67.01          a1 = 25.677         b1 = -13.0
// L2 = 66.7           a2 = 50.16          b2 = -16.63
// CIE ΔE2000 = ΔE00 = 9.0634964928

// L1 = 26.0           a1 = -26.1481       b1 = -45.9326
// L2 = 33.8265        a2 = -17.7          b2 = -18.0
// CIE ΔE2000 = ΔE00 = 12.45795704788

// L1 = 99.927         a1 = 25.2294        b1 = -86.501
// L2 = 79.745         a2 = 17.0           b2 = -53.0
// CIE ΔE2000 = ΔE00 = 15.58327744506

// L1 = 76.0           a1 = 78.0           b1 = 84.36
// L2 = 61.49          a2 = 32.25          b2 = 54.8
// CIE ΔE2000 = ΔE00 = 18.212467728

// L1 = 26.9           a1 = -9.0           b1 = 115.9
// L2 = 20.168         a2 = -67.9          b2 = 126.4
// CIE ΔE2000 = ΔE00 = 21.44140370993

// L1 = 70.0           a1 = 92.9           b1 = 24.0329
// L2 = 34.9           a2 = 105.7          b2 = 101.0
// CIE ΔE2000 = ΔE00 = 42.73724078015
...
fun rgb_to_xyz(ra: Double, ga: Double, ba: Double): DoubleArray {
	...
	val r = if (ra < 0.040448236276933) ra / 12.92 else Math.pow((ra + 0.055) / 1.055, 2.4)
	val g = if (ga < 0.040448236276933) ga / 12.92 else Math.pow((ga + 0.055) / 1.055, 2.4)
	val b = if (ba < 0.040448236276933) ba / 12.92 else Math.pow((ba + 0.055) / 1.055, 2.4)
...
fun xyz_to_rgb(x: Double, y: Double, z: Double): DoubleArray {
	...
	r = if (r < 0.0031306684424956) 12.92 * r else 1.055 * Math.pow(r, 1.0 / 2.4) - 0.055
	g = if (g < 0.0031306684424956) 12.92 * g else 1.055 * Math.pow(g, 1.0 / 2.4) - 0.055
	b = if (b < 0.0031306684424956) 12.92 * b else 1.055 * Math.pow(b, 1.0 / 2.4) - 0.055
...
// The goal of this demo is to use the CIEDE2000 function to compare two RGB colors.

fun main() {
	// 1. Define two RGB colors
	val rgb_1 = listOf(0, 0, 139)  // darkblue
	val rgb_2 = listOf(0, 0, 128)  // navy

	// 2. Normalize RGB values (0–255 → 0.0–1.0) and convert them to L*a*b* color space
	...
	// 3. Calculate the color difference using the CIEDE2000 ΔE formula
	...
	// 5. Display the L*a*b* values with formatting to 2 decimal places
	...
	// 6. Display the final Delta E value with 4 decimal places
	println("Delta E (CIEDE2000) = ${"%.4f".format(delta_e)}")

	// The result shows a small color difference: ΔE2000 ≈ 1.56
}
