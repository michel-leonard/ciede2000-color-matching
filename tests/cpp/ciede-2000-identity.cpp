...
// This file serves as a precision validator between the C and C++ implementations
...
int main(int argc, char *argv[]) {
	...
	std::fprintf(stdout, "                Validates 32-bit and 64-bit implementations (C++ and C).\n\n");
	...
	std::fprintf(stdout, "Algorithm     : State-of-the-art Delta E 2000 (passes 100B comparisons vs Netflix VMAF)\n");
	...
	std::fprintf(stdout, "Tolerance (float) : %.2e       (C++ vs C)\n", tolerance_32_bits);
	std::fprintf(stdout, "Tolerance (double): %.2e       (C++ vs C)\n\n", tolerance_64_bits);
		...
		for (long i = 0; i < n_iterations; ++i) {
			...
			if (tolerance_64_bits < deviation) {
				...
				if (chars_printed < error_print_length) {
					...
					chars_printed += std::fprintf(stderr, "       Delta E in C : %-18.15f    (deviation: %.1e)\n", delta_1, deviation);
	...
	std::fprintf(stdout, "Double precision test %s in %.02f seconds\n", n_err_64 ? "failed" : "passed", (double) (t_4 - t_3) / 1e9);
		...
		for (long i = 0; i < n_iterations; ++i) {
			...
			if (tolerance_32_bits < deviation) {
				...
				if (chars_printed < error_print_length) {
					...
					chars_printed += std::fprintf(stderr, "  Delta E in C : %-13.7f    (deviation: %.1e)\n", delta_1, deviation);
	...
	std::fprintf(stdout, "Float precision test %s in %.02f seconds\n", n_err_32 ? "failed" : "passed", (double) (t_6 - t_5) / 1e9);
	std::fprintf(stdout, "  - Avg deviation  : %.2e\n", avg_deviation_32_bits / (float) n_iterations);
	std::fprintf(stdout, "  - Max deviation  : %.2e\n", max_deviation_32_bits);
	std::fprintf(stdout, "  - Errors         : %" PRId64 "\n\n", n_err_32);
	if (!n_err_32 && !n_err_64) {
		std::fprintf(stdout, "-- Performance Summary --\n");
		std::fprintf(stdout, "Double: %.02f M calls/sec\n", (double) n_iterations / (double) (t_4 - t_3) * 2e3);
		std::fprintf(stdout, "Float : %.02f M calls/sec\n", (double) n_iterations / (double) (t_6 - t_5) * 2e3);
		std::fprintf(stdout, "Speedup (float vs double): %.02fx\n\n", (double) (t_4 - t_3) / (double) (t_6 - t_5));
		std::fprintf(stdout, "Status: PASS - All implementations match within strict tolerances.\n");
	} else
		std::fprintf(stdout, "Status: FAIL - Results diverge between C++ and C (see above).\n");
	return 0;
}

// In addition, this workflow :
// - Ensures that any deviation across 30+ implementations is reliably detected.
// - Catches regressions between the C and C++ implementations.
