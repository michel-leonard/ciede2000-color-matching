...
ciede_2000(L1, A1, B1, L2, A2, B2, DeltaE2000) :-
	...
	(1.0e-14 < Wrap_dist, Pi_1 < Delta_h -> Hue_wrap = 1 ; Hue_wrap = 0),
	H_mean is H_mean_raw + Hue_wrap * Pi_1,
	(Hue_wrap =:= 1, H_diff_raw < 0.0 -> H_diff_hi = Pi_1 ; H_diff_hi = 0.0),
	(Hue_wrap =:= 1, H_diff_hi =:= 0.0 -> H_diff_lo = Pi_1 ; H_diff_lo = 0.0),
	H_diff is H_diff_raw + H_diff_hi - H_diff_lo,
...
% L1 = 50.2           a1 = -113.0         b1 = -78.2
% L2 = 54.3937        a2 = -113.0         b2 = -84.9
% CIE ΔE2000 = ΔE00 = 4.40208739193
...
% in Prolog displays the original line with the computed Delta E 2000 color difference appended.
...
main :-
	...
	( Argv = [Filename|_] ->
		...
		close(Stream)
	; 
		writeln('Usage: swipl -q -s ciede-2000-driver.pro -- <filename>'),
		halt(1)
	).

process(Line) :-
	split_string(Line, ",", " ", Explode),
	maplist(number_string, [L1,A1,B1,L2,A2,B2], Explode),
	ciede_2000(L1, A1, B1, L2, A2, B2, DeltaE2000),
	format("~s,~w~n", [Line, DeltaE2000]).
