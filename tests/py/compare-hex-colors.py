...
# L1 = 86.9   a1 = 25.7   b1 = -3.8
# L2 = 87.5   a2 = 20.0   b2 = 3.6
# CIE ΔE00 = 5.7061560876 (Bruce Lindbloom, Netflix’s VMAF, ...)
# CIE ΔE00 = 5.7061391242 (Gaurav Sharma, OpenJDK, ...)
...
def rgb_to_xyz(r, g, b):
	...
	r = math.pow((r + 0.055) / 1.055, 2.4) if r > 0.040448236276933 else r / 12.92
	g = math.pow((g + 0.055) / 1.055, 2.4) if g > 0.040448236276933 else g / 12.92
	b = math.pow((b + 0.055) / 1.055, 2.4) if b > 0.040448236276933 else b / 12.92
...
def xyz_to_rgb(x, y, z):
	...
	r = 1.055 * math.pow(r, 1.0 / 2.4) - 0.055 if r > 0.0031306684424956 else 12.92 * r
	g = 1.055 * math.pow(g, 1.0 / 2.4) - 0.055 if g > 0.0031306684424956 else 12.92 * g
	b = 1.055 * math.pow(b, 1.0 / 2.4) - 0.055 if b > 0.0031306684424956 else 12.92 * b
...
# The goal of this demo in Python is to use the CIEDE2000 function to compare two hexadecimal colors.

# Step 1: Define two HEX color codes
hex_1 = "#00ff00"   # Pure green
hex_2 = "#00c800"   # Slightly darker green

# Step 2: Convert HEX colors to RGB tuples
rgb_1 = hex_to_rgb(hex_1)  # e.g., (0, 255, 0)
rgb_2 = hex_to_rgb(hex_2)  # e.g., (0, 200, 0)

# Step 3: Normalize RGB values (scale 0–255 to 0–1) and convert to CIE Lab color space
lab_1 = rgb_to_lab(rgb_1[0] / 255.0, rgb_1[1] / 255.0, rgb_1[2] / 255.0)
lab_2 = rgb_to_lab(rgb_2[0] / 255.0, rgb_2[1] / 255.0, rgb_2[2] / 255.0)

# Step 4: Calculate color difference (ΔE) using the CIEDE2000 formula
delta_e = ciede_2000(lab_1[0], lab_1[1], lab_1[2],
		lab_2[0], lab_2[1], lab_2[2])

# Step 5: Display original HEX codes
print(f"Color 1 HEX: {hex_1}")
print(f"Color 2 HEX: {hex_2}")

# Step 6: Display converted RGB values
print(f"Color 1 RGB: {rgb_1}")
print(f"Color 2 RGB: {rgb_2}")

# Step 7: Display converted Lab values with formatted precision
print(f"Color 1 Lab: (L={lab_1[0]:.2f}, a={lab_1[1]:.2f}, b={lab_1[2]:.2f})")
print(f"Color 2 Lab: (L={lab_2[0]:.2f}, a={lab_2[1]:.2f}, b={lab_2[2]:.2f})")

# Step 8: Display the calculated color difference
print(f"Delta E (CIEDE2000) = {delta_e:.4f}")

# This shows a ΔE2000 of 12.58
