...
The `ciede_2000` functions produce the same result regardless of the order in which the two colors are provided. This **symmetry property**, essential for comparators, is enforced at the core level by the C driver, which generates colors along with their permutations and asserts the symmetry. As a result, all programming languages are tested within their respective workflows to ensure the symmetry of the function.

## Test Samples

The `logs.txt` file presents the results of testing the `ciede_2000` function across 14 programming languages. This table contains an extract of the 1,365,000,000 test samples that were originally used to validate implementations of the CIE 2000 color difference formula. Each row contains two colors in L* a* b* format, and the test is successful if the difference between ΔE2000 does not exceed `1e-10`.
...
> [!TIP]
> A web-based [discovery generator](https://github.com/michel-leonard/ciede2000-color-matching?tab=readme-ov-file) is available for exploration, and a [large-scale validator](https://michel-leonard.github.io/ciede2000-color-matching) is in use to validate your implementation.
...
All 30+ programming languages ​​are also tested individually using [workflows](../.github/workflows/README.markdown#δe2000-workflows).
...
This implementation of the ΔE CIE2000 color difference formula has been validated against billions of color pair comparisons, demonstrating exceptional accuracy and numerical stability. Compared to established libraries—C99's **VMAF** (2016), npm's **delta-e** (2014), Python's **colormath** (2008), and Rust's **palette** (2015), it shows an absolute deviation of no more than 10⁻¹², confirming its correctness across multiple programming languages.
