...
Compared to the widely used Rust crate **palette** — first released in 2015 and still actively maintained 10 years later — this implementation of the ΔE2000 color difference formula has been validated against billions of color pairs. The observed absolute deviation never exceeds 5 × 10⁻¹³, confirming the accuracy of the **generic precision** algorithm, as previously demonstrated in [C99](../c#comparison-with-the-vmaf-c99-library), [JavaScript](../js/README.markdown#comparison-with-the-npmdelta-e-library) and [Python](../py#comparison-with-the-python-colormath-library).
